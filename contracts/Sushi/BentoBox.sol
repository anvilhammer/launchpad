...
contract BentoBox is MasterContractManager, BoringBatchable {
    ...
    IERC20 private immutable wadaToken;
    ...
    constructor(IERC20 wadaToken_) public {
        wadaToken = wadaToken_;
    }
    ...
    function deposit(
        ...
        // Effects
        IERC20 token = token_ == USE_ETHEREUM ? wadaToken : token_;
        ...
        if (token_ == USE_ETHEREUM) {
            ...
            // X2: If the WADA implementation is faulty or malicious, it will block adding ETH (but we know the `ADA implementation)
            IWADA(address(wadaToken)).deposit{value: amount}();
    ...
    function withdraw(
        ...
        // Interactions
        if (token_ == USE_ETHEREUM) {
            // X2, X3: A revert or big gas usage in the WADA contract could block withdrawals, but WADA9 is fine.
            IWADA(address(wadaToken)).withdraw(amount);
